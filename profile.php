  ...
  <section class="trending-article">
    ...
    <article class="article">
      ...
      <div class="user-manage">
        <div class="user-info">
          <p>Username: <a href="profile.php?username=<?= rawurlencode($user['username'])?>"><?= $user['username'] ?></a></p>
    ...
    <?php else : ?>
      <div class="manage-links">
        <?php if (isset($_SESSION['user']) && $_SESSION['user']['role'] === 'admin' && $_SESSION['user']['user_id'] === $user['user_id']): ?>
          <a class="logout-btn" href="manageUser.php">Manage other users</a>
        <?php endif ?>
      </div>
        ...
        <?php for ($i = 0; $i < count($allPosts); $i++) : ?>
          <article class="article">
            <div class="article-img">
              <img src="img/<?= rawurlencode($allPosts[$i]['thumbnail']) ?>" alt="article img" width="100" height="350">
            </div>
            <div class="article-info">
              ...
              <a class="category-btn" href="category.php?category=<?= rawurlencode($allPosts[$i]['category']) ?>"><?= $allPosts[$i]['category'] ?></a>
              <p class="article-description">
                <?= substr($allPosts[$i]['body'], 0, 300) . '  ...' ?>
              </p>
              <div class="article-data">
                <div class="author">
                  <p><a class="article-author" href="profile.php?username=<?= rawurlencode($allPosts[$i]['username'])?>"><?= $allPosts[$i]['username'] ?></a></p>
      ...
      <div class="pagination">
        <ul>
          ...
          <?php else: ?>
            <li><a href="?username=<?=rawurlencode($username)?>&page=<?= $page - 1?>">Prev</a></li>
          <?php endif ?>
          <?php for($i = 0; $i < count($pageLinks); $i++): ?>
              <?php if($pageLinks[$i]['data'] === $page) : ?>
                <li><a href="?username=<?=rawurlencode($username)?>&page=<?= $page ?>" class="active"><?=$pageLinks[$i]['data']?></a></li>
              <?php else : ?>
                <li><a href="?username=<?=rawurlencode($username)?>&page=<?= $pageLinks[$i]['data'] ?>"><?=$pageLinks[$i]['data']?></a></li>
              <?php endif ?>
          ...
          <?php else: ?>
            <li><a href="?username=<?=rawurlencode($username)?>&page=<?= $page + 1?>">Next</a></li>
          <?php endif ?>
        </ul>
      </div>
    <?php endif ?>
